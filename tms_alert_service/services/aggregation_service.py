...
from tms_alert_service.db import db
from tms_alert_service.metrics import PENDING_AGGREGATION
from tms_alert_service.models import N9eEvent
from tms_alert_service.services.time_utils import now_in_tz
from tms_alert_service.services.webhook_service import send_webhook
...
async def flush_aggregations_once() -> None:
    ...
    fetch_sql = """
    SELECT id, bucket_start, hostname, service_name, status, robot_token, session, service_owner, mobile, count
    ...
    with db.conn() as conn:
        with conn.cursor() as cur:
            ...
            PENDING_AGGREGATION.set(pending)
    ...
    for row in rows:
        ...
        owner_info = row['service_owner'] or '未知负责人'
        phone_info = row['mobile'] or '未配置手机号'
